...
class MultiScaleDWConv(nn.Module):
    def __init__(self, dim, scale=(3, 5, 7, 9,11)):
        super().__init__()
        self.scale = scale
        self.channels = []
...
class MultiScaleAttention(nn.Module):
    def __init__(
        ...
        self.LKA = LKA(dim)
        self.Conv = nn.Conv2d(dim, dim, 3, padding=3 // 2, groups=dim)
        if pool_first:
            self.q = nn.Linear(dim, dim_out, bias=qkv_bias)
            self.k = nn.Linear(dim, dim_out, bias=qkv_bias)
            self.v = nn.Linear(dim, dim_out, bias=qkv_bias)
        else:
            self.qkv = nn.Linear(dim, dim_out * 2, bias=qkv_bias)
    ...
    def forward(self, x, hw_shape):
        B, N, C = x.shape
        H = W = int(N ** 0.5)
        x_0 = x.reshape(B, H, W, C).permute(0, 3, 1, 2)
        x = self.Conv(x_0).permute(0, 2, 3, 1).reshape(B, N, C)
        q = self.LKA(x_0).permute(0, 2, 3, 1).reshape(B, N, C)
        if self.pool_first:
            if self.mode == "conv_unshared":
                fold_dim = 1
            else:
                fold_dim = self.num_heads
            x_1 = x.reshape(B, N, fold_dim, -1).permute(0, 2, 1, 3)
            k = v = x_1
        else:
            assert self.mode != "conv_unshared"

            qkv = (self.qkv(x).reshape(B, N, 2, self.num_heads, -1).permute(2, 0, 3, 1, 4))
            k, v = qkv[0], qkv[1]
